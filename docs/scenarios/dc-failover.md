...
**⚠️ TTL Trade-off:** Клиенты с кэшированным DNS (TTL=300 сек) будут пытаться подключиться к EU DC ещё до 5 минут после сбоя. Полный failover = до TTL. Для критичных систем используют короткий TTL (30-60 сек) + health checks на клиенте.
